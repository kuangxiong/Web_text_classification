...
# ernie模型的配置文件
from models.ErnieBilstm import ErnieModelConfig
...
def ernie_get_text_id(ErnieModelConfig, train_text, max_length=ErnieModelConfig.max_len):
    """
    将训练集上的文字字符转化为字编号
    ...
    """
    token_dict = {}
    with codecs.open(ErnieModelConfig.ernie_vocab_path, 'r', 'utf8') as reader:
        for line in reader:
            token = line.strip()
            token_dict[token]= len(token_dict)
    
    tokenizer = Tokenizer(token_dict)
    data_X_ind, data_X_seg, data_Y = [], [], []
    N = len(train_text)
    for i in range(N):
        tmp_text = train_text[i][1]
        indices, segments = tokenizer.encode(first=tmp_text, max_len=ErnieModelConfig.max_len)
    ...
    padding='post', truncating='post', value=0.)
#data_X = [[np.array(e1), np.array(e2)] for e1, e2 in zip(data_X_ind, data_X_seg)] 
    return data_X_ind, data_X_seg, data_Y 
    
def load_ernie_dataset(GlobalConfig, max_length = ErnieModelConfig.max_len):
    """
    生成训练数据集合
    ...
    train_text, train_seg, train_label_id = ernie_get_text_id(ErnieModelConfig, train_data,
...
def ernie_get_X_and_Y_data(data_text, data_label):
    """
    [将数据分为训练集数据和验证集数据]
...
#    train_data, test_data = data_load(GlobalConfig)
#    print(train_data[:5])
#tmp1, tmp2 = ernie_get_text_id(ErnieModelConfig, train_data)
#    print(tmp1[5])
#    print(tmp2[5])
    train_text, _, train_label = load_ernie_dataset(GlobalConfig)
