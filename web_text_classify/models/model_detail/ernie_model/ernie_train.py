...
from models.ErnieBilstm import ernie_bilstm, ErnieModelConfig
from data_preload import load_model_dataset, get_X_and_Y_data
from ernie_data_preload import load_ernie_dataset
from config import GlobalConfig
...
if __name__=='__main__':
    ...
    train_data, train_seg, train_label = load_ernie_dataset(GlobalConfig,
            max_length = ErnieModelConfig.max_len) 
...
#    model_file = os.path.join(GlobalConfig.save_path, "erniemodel.h")
#    if os.path.isfile(model_file):
#        model = keras.models.load_model(model_file)
#    else:
    model = ernie_bilstm(ErnieModelConfig)
    adam = tf.keras.optimizers.Adam(ErnieModelConfig.learning_rate)
    model.compile(
...
#    print(train_X_seg.shape, train_X_seg.dtype)
    history = model.fit( 
        ...
        batch_size=ErnieModelConfig.batch_size, # 每一批batch的大小
